...
class RoiPdfGenerator(private val context: Context) {
    ...
    private fun createPdfInternal(state: RoiState, mode: PdfMode) {
        ...
        fun drawHeader() {
            ...
            val title = if (mode == PdfMode.COUNTER_OFFER) "Counter Offer Proposal" else "${state.tenantName.ifBlank { "Property" }} ROI Calculation"
        ...
        fun checkSpace(heightNeeded: Float) {
            if (y + heightNeeded > BOTTOM_LIMIT) startNewPage()
        }
        ...
        fun formatCurrency(amount: Any?): String {
            ...
            if (d % 1.0 == 0.0) formatter.maximumFractionDigits = 0 else formatter.maximumFractionDigits = 2
        ...
        val escText = if (state.escalationPercent.isNotBlank() && state.escalationYears.isNotBlank()) "${state.escalationPercent}% every ${state.escalationYears} Years" else "-"
        ...
        val registry = state.registryCost
        ...
        drawRow("Registry", formatCurrency(registry))
        ...
        checkSpace(160f) // ⚡ Increased space for legal text
        val rightEdge = PAGE_WIDTH - MARGIN
        val boxPaint = Paint()
        ...
        if (mode == PdfMode.COUNTER_OFFER) {
            ...
            paint.textSize = 36f
            paint.typeface = Typeface.DEFAULT_BOLD
            canvas.drawText(String.format("%.2f%%", state.calculatedRoi), (PAGE_WIDTH / 2).toFloat(), y + 70f, paint)
        }
        ...
        val legalText = if(state.isBuyerMode) {
            "For Buyer: Security deposit of Rs ${formatCurrency(state.securityDeposit)} to be received by the new buyer from seller."
        } else {
            "For Seller: Security Deposit of Rs ${formatCurrency(state.securityDeposit)} to be given to the Buyer by Seller."
        }
        canvas.drawText(legalText, MARGIN, y, paint)
